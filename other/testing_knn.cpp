...
int main(void) {
    ...
    // CUSTOM PARAMS with variable k
    int ref_nb   = 16384;
    int query_nb = 4096;
    int dim      = 128;
    int k[]      = {1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024};
    ...
    for(int i=0; i<sizeof(k)/sizeof(k[0]); i++){
        ...
        float * knn_dist   = (float*) malloc(query_nb * k[i]   * sizeof(float));
        int   * knn_index  = (int*)   malloc(query_nb * k[i]   * sizeof(int));
        ...
        if (!knn_c(ref, ref_nb, query, query_nb, dim, k[i], knn_dist, knn_index)) {
            free(ref);
            free(query);
            free(knn_dist);
            free(knn_index);
        ...
        test(ref, ref_nb, query, query_nb, dim, k[i], knn_dist, knn_index, &ys_for_param2,  "p2_ready", 50);

        test(ref, ref_nb, query, query_nb, dim, k[i], knn_dist, knn_index, &ys_gpu_partial_sort,  "ps_gpu", 50);
